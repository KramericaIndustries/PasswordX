...
if (is_object($uo)) { 
	...
	<div class="ccm-pane-options">
		...
		<?php 
		//Fuck you, spacer.gif, just fuck you!
		//echo $av->outputUserAvatar($uo);
		?>
		
		<!-- Lets step away a bit from the way C5 handles this view -->
		<h3><?php echo t('User Details')?></h3><br>
		
		<div class="row">
			<div class="span2" style=""><p><strong>Username:</strong></p></div>
			<div class="span8"><p><?php echo $uo->getUserName()?></p></div>
		</div>
		
		<div class="row">
			<div class="span2" style=""><p><strong>Full Name:</strong></p></div>
			<div class="span8"><p><?php echo $uo->getAttribute('real_name'); ?></p></div>
		</div>

		<div class="row">
			<div class="span2" style=""><p><strong>Email Address:</strong></p></div>
			<div class="span8"><p><a href="mailto:<?php echo $uo->getUserEmail()?>"><?php echo $uo->getUserEmail()?></a></p></div>
		</div>
		
		<div class="row">
			<div class="span2" style=""><p><strong>Phone Number:</strong></p></div>
			<div class="span8"><p>
				<?php 
				$phone_number = sprintf( "(%s) %s", $uo->getAttribute('phone_country_code'), $uo->getAttribute('phone_number') );
				echo ($phone_number != '()' ) ? 'N/A' : $phone_number; 
				?>
			</p></div>
		</div>
		
		<div class="row">
			<div class="span2" style=""><p><strong>Authy UserID:</strong></p></div>
			<div class="span8"><p><?php $authy_id = $uo->getAttribute('authy_user_id'); echo empty($authy_id) ? 'N/A' : $authy_id; ?></p></div>
		</div>
		<hr />
		<!-- //Done -->
		
		<!-- Concrete5 user details
		...
		<!-- Pages that teh user can access -->
		...
		<!-- Small logs -->
		<h3>Significant Logs</h3><br>
		<div class="row">
			<div class="span2" style=""><p><strong>Account created:</strong></p></div>
			<div class="span8"><p><?php echo date(DATE_APP_GENERIC_MDYT, strtotime($uo->getUserDateAdded('user'))); ?></p></div>
		</div>
		
		<div class="row">
			<div class="span2" style=""><p><strong>Last Login:</strong></p></div>
			<div class="span8"><p><?php echo date(DATE_APP_GENERIC_MDYT, $uo->getLastLogin()); ?></p></div>
		</div>
		
		<div class="row">
			<div class="span2" style=""><p><strong>Last Login Location:</strong></p></div>
			<div class="span8"><p>
				<?php echo $uo->getLastIPAddress(); ?> - 
				<?php 
					$nsa = Loader::helper('nsa');
					$geoIp = $nsa->geoLocateIP( $uo->getLastIPAddress() );
					echo (is_object( $geoIp ) ? sprintf( "%s, %s, %s", $geoIp->city, $geoIp->region_name, $geoIp->country_name ) : "unknown location");
				?>
			</p></div>
		</div>		
		...
		$attribs = UserAttributeKey::getList(true);
		if (count($attribs) > 0) { ?>
		<h3><?php echo t('Misc Attributes')?></h3><br/>

		<?php 
		$hidden_attributes = array( 'real_name', 'phone_number', 'phone_country_code', 'authy_user_id' );
		for ($i = 0; $i < count($attribs); $i++) { 			
			$uk = $attribs[$i]; 
			if( in_array( $uk->getAttributeKeyHandle(), $hidden_attributes ) ) {
				continue;
			}
