...
<div class="ccm-ui" style="padding-bottom: 20px;">
<div class="row">
...
<div class="ccm-pane-body">
	...
	<h3 style="margin-bottom: 10px;"><img src="<?php echo $ci->getBlockTypeIconURL($bt)?>" /> <?php echo t($bt->getBlockTypeName())?></h3>
		
	<h5><?php echo t('Description')?></h5>
	<?php if( $bt->getBlockTypeDescription() == null ) { ?>
		<p><i><?php echo t("No description available");?></i></p>
	<?php } else { ?>
	...
	<h5><?php echo t('Encryption Support')?></h5>
	<p><?php echo (substr($bt->getBlockTypeHandle(),0,10) == "encrypted_") ? "Yes" : "No"; ?></p>
	...
	<div id="ccm-block-type-list-installed" class="ccm-block-type-sortable-list">
		<?php foreach($normalBlockTypes as $bt) { 
			...
			<div class="ccm-group" id="btID_<?php echo $btID?>" data-btid="<?php echo $btID?>">
				...
				<?php $encr_icon = (substr($bt->getBlockTypeHandle(),0,10) == "encrypted_" ? '<i class="icon-eye-close" alt="Block Supports Encryption" title="Block Supports Encryption"></i>' : ''); ?>
				<a class="ccm-group-inner" href="<?php echo $this->action('inspect', $bt->getBlockTypeID())?>" style="background-image: url(<?php echo $btIcon?>)"><?php echo t($bt->getBlockTypeName())?> <?php echo $encr_icon ?></a> 
			</div>
		<?php } ?>
	</div>
	<script type="text/javascript">
