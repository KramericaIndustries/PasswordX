...
/**
 * Massive override as this is not subclassed - objective: make the "Back to Dashboard" link consistent with PasswordX UI
 * (c) 2014 PasswordX
 * Apache v2 License
 */
		...
		if (count($trail) > 1 || count($navigatePages) > 1 || is_object($upToPage)) { 
			...
			if (count($subpagesP) > 0 || is_object($upToPage)) { 
				...
				if ($upToPage) { 
					...
					//PasswordX tweak
					 $relatedPages .= '<li><a href="/index.php/dashboard/passwordx/administration/">' . t('&lt; Back to %s', t('Dashboard')) . '</a></li>';
